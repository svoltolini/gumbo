...
## Reviewed SMB album deletion

SMB now advertises exact-file deletion separately from upload/tag replacement. Preparing a review opens each file with read plus DELETE access, denies SMB write/delete sharing, refuses directory/reparse targets, and fingerprints every byte with SHA-256 plus file identity/change attributes. Confirmation repeats this check through a new protected handle, rechecks the owner/connection/root/revision authority, and sends `FileDispositionInformation` against that same handle. The original pathname is not reopened for deletion. There is no recursive directory operation, fallback unlink, or destructive reconnect/retry.

A successful SET_INFO and CLOSE are required before catalogue/download/Watch cleanup. An interrupted reply reports uncertainty and stops the batch, including when Stop is also pressed; the app does not infer success from a subsequently missing path. The normal connection remains available for playback while a separate transaction verifies a file. Verification is chunked, cancellable and bounded to 120 seconds per attempt (the existing read-only connection recovery may retry once). A busy file has a specific explanation; permission checks use server NTSTATUS rather than stale process errno.

The Release core run passed **704 tests in 55 suites**, with the loopback Samba integration enabled. The C receive/directory/entropy checks and 48,000-context lifecycle regression also passed. The source patch applies cleanly to the pinned upstream revision. Signed Release builds passed for iOS (including Watch/widgets), macOS and tvOS. All app signatures verified; iOS/Mac/TV still link the separate SMB framework with the secure entropy import, and Watch contains no SMB binary. These are build/signature checks, not new TestFlight uploads or physical-device journeys.

The disposable Samba fixture covers unchanged reviewed deletion, changed same-size files, existing writers, competing write/rename/delete at final authorization, owner revocation, read-only share/directory refusal, and a real reply-loss relay after server acceptance. Library tests verify partial completion, uncertain-result batch stopping, cancellation acknowledgements, profile/source changes and exact-file catalogue/download cleanup. No user-library data is changed. These tests establish the Samba protocol path, not NAS model certification or physical-device acceptance.

Native SMB tag replacement remains unavailable; generic WebDAV mutations remain disabled after the earlier failure experiment. The later protected inspection and optional helper work below supersedes the earlier damaged-file cleanup limitation. Non-SMB/local writers may bypass SMB share locks: before/after attributes detect observed changes, but the file server's sharing and filesystem behavior remain a deployment prerequisite. No universal transaction guarantee is claimed for arbitrary server-side aliases or noncooperating local writers. Remaining work stays in #196 and #198.

## Provider maintenance completion

The owner can now review and delete damaged files from SMB using inspection reads and final validation on a protected SMB handle. Read-only inspection cannot manufacture a deletion witness. Lost acknowledgements stop the batch even after cancellation; confirmed file removals are applied and persisted individually, with local persistence errors reported separately.

The optional metadata helper adds reviewed album deletion and damaged-file cleanup to read-only transports. Deletion defaults to off in the server and app. The app cross-checks mapped file contents through both connections; explicit operator root mapping remains required because identical copied libraries cannot establish physical path identity. Helper inspection captures bounded bytes during the exact full-file hash pass and refuses a changed fingerprint. The deletion engine reserves a private recovery directory, captures the candidate, checks its inode/content and only then unlinks it. Changed candidates are restored without overwriting a new source file; lost durable acknowledgement is unconfirmed and stops the job. The app retains that original job reference. Read-only, locked-owner and family-member permissions, helper/root/source changes, empty files, changed bytes and cancellation are covered.

Validation on 21 September 2026: **719 Release core tests in 56 suites**, with actual loopback Samba tests enabled; **25 helper tests on macOS and in a restricted Linux container**; signed Release builds for iPhone/iPad (including Watch/widgets), Mac and Apple TV. Main apps and embedded SMB frameworks pass deep/strict signature checks. iOS retains Siri and CarPlay entitlements; Watch contains no SMB framework. The temporary Samba containers were stopped after testing. No user-library files were used for destructive tests.

The configuration preflight has 16 passes and three existing encryption-documentation blockers, not a green upload check. These builds use the existing version/build as a source/signing proof, not a new TestFlight upload. Native WebDAV DELETE/MOVE and SMB tag replacement remain disabled; the optional helper supplies those maintenance features through its local filesystem contract. Advisory/noncooperating filesystem-writer limits remain explicit in the helper documentation. Named NAS/device acceptance stays in #123/#198, actual helper installation and LAN/remote timing in #201, and France release documentation in #208.
