...
| Profile sync | Name, chosen photo, role, dates, Apple user-record association, PBKDF2 PIN verifier in `record.encryptedValues` (plain fields only mark that a PIN is set); favourites, playlists, searches, recent plays and settings in private/family-shared CloudKit zone | `CloudSync.record(for:)`, `CloudSync.writePIN`, `Models/Profile.swift`, `ProfileStateMerge.swift`, [PIN verifier deployment](CLOUDKIT-PIN-VERIFIER-DEPLOYMENT.md) |
| Family connection | NAS address, names, account and music folder in family CloudKit record; selected Family Access password in `record.encryptedValues`; anyone with the share link can join | `CloudSync.record(for: FamilyRecordPlan)`, `FamilyRecordPlan.swift` |
