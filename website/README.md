...
- **Brand and products:** Gumbo is the text-only umbrella wordmark. Music stays at `/`; Film has a static `/film/` page marked **In development**. The single-row header uses a compact native product disclosure with product logos, full names and statuses. Its ordinary links use `aria-current="page"` and work without JavaScript. On mobile, the beta link sits in the section menu as well as the page content. Keep product logos, copy, favicons and social cards specific to their product. Add future products as separate pages using the same shell; do not promise unconfirmed features or release dates.
- **Screenshots:** replace each entire decorative mockup `<div>` with `<img class="desktop-preview product-screenshot">` for Mac or `<img class="phone-preview product-screenshot">` for iPhone. Add `src`, descriptive `alt`, and natural `width`/`height`. The visible preview caption has been removed. HTML comments mark both slots; the Mac and iPhone visuals remain illustrative mockups. The figure has an accessible description. `assets/gumbo-watch-player-artwork.png` is an unedited 416 × 496 Watch simulator capture of the native `NowPlayingView` used by Gumbo, seeded in an isolated fixture with built-in sample-library metadata (Copper Wire / Halden Vey) and silent local audio. It uses the same metadata fields and native artwork callback as the app. The cover fixture uses the existing Nocturne Drift generated gradient from Gumbo’s sample ArtworkView; the shipping app sends small thumbnails only from its local NAS-derived cover cache, and keeps the native placeholder if a cover is unavailable. It is presented upright in the Apple Watch device section with a slim screen border; no simulated straps, crown or rotation. Do not replace it with an invented interface or imply this sample capture verifies a real NAS connection.
...
The product selector and mobile menu use native `<details>` and `<summary>` with a shared `name`, so both work without JavaScript and open one at a time. The script adds dismissal on section selection, outside click, Escape (with focus returned), focus leaving the menu, and switching to desktop width. Check these behaviors with a keyboard and at narrow widths after changing the header.
