...
- **Screenshots:** replace each entire decorative mockup `<div>` with `<img class="desktop-preview product-screenshot">` for Mac or `<img class="phone-preview product-screenshot">` for iPhone. Add `src`, descriptive `alt`, and natural `width`/`height`. The visible preview caption has been removed. HTML comments mark both slots; the Mac and iPhone visuals remain illustrative mockups. The figure has an accessible description. `assets/gumbo-watch-player.png` is an unedited 416 × 496 Watch simulator capture of the native `NowPlayingView` used by Gumbo, seeded in an isolated fixture with built-in sample-library metadata (Copper Wire / Halden Vey) and silent local audio. It uses the same metadata fields as the app, including the system placeholder because the Watch player does not currently receive cover images. It is presented upright in the Apple Watch device section with a slim screen border; no simulated straps, crown or rotation. Do not replace it with an invented interface or imply that the Watch receives artwork.
