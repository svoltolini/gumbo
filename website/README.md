...
- **Social image:** `assets/gumbo-music-social-v2.png` is the production **1200 × 630** branded card; its editable source is `assets/og-image.svg`. Render that SVG at its natural size after editing. Use a new image filename for future revisions and update both Open Graph and Twitter image URLs together so refreshed previews request the new asset. `assets/og-image.png` is retained as a compatibility copy for older links. `assets/logo.svg` is the supplied logo.
- **Favicons:** `favicon.svg` uses the original Gumbo mark with less surrounding padding for small sizes. `favicon.ico` contains 16, 32 and 48px versions; `apple-touch-icon.png` is an opaque 180 × 180 image. All are linked in the HTML head with absolute production URLs.
