...
- Replaces the source atomically in the same directory, with an original-inode recovery link until the successful result is durable. A verification or storage failure after replacement rolls back when the target is still the helper's replacement. If another writer changed it, the rollback itself fails or cannot be made durable, or acknowledgement cannot be persisted, the helper retains the original recovery copy and reports **unconfirmed**.
...
Failed and unconfirmed outcomes, and error responses, carry `error: {code, message}`. Storage errors are reported by cause, never as a path problem: `unsafe_or_missing_path` (missing file or folder, or a symbolic link), `permission_denied` (the helper's user can't write the file or folder, or the share is read-only), `insufficient_space` (no space or quota; HTTP 507), `file_busy` (another program holds the file), `io_error` (other storage errors such as EIO; HTTP 500), plus `conflict`, `unsafe_file`, `ownership_mismatch`, `verification_failed` and `cancelled`. These leave the original in place and later files continue. `recovery_required` marks an **unconfirmed** file: the job stops and remaining files are cancelled.
